...
namespace Agenda.Crud.Controllers;
...
public class HomeController : Controller
{
    ...
    public List<ScheduleEvent> LoadData([FromBody] Params param)
    {
        ...
        var resultado = _context.ScheduleEvents.Where(app => (app.StartTime >= start && app.StartTime <= end) || (app.RecurrenceRule != null && app.RecurrenceRule != "")).ToList(); // Here filtering the events based on the start and end date value.
    ...
    public List<ScheduleEvent> UpdateData([FromBody] EditParams param)
    {
        if (param.action == "insert" || (param.action == "batch" && param.added.Count > 0)) // this block of code will execute while inserting the appointments
        {
            ...
            {
                ScheduleEvent value = (param.action == "insert") ? param.value : param.added[i];
